...
# ---------- gene methylation PCA by %

get_percent_modified <- function(df, filter_label) {
  df <- df[df$label == filter_label, c("ID", "canonical_wam")]
  rownames(df) <- df$ID
  df$ID <- NULL
  return(df)
}

df_28 <- read.delim(wam_analysis_28hpi, header=TRUE)
df_32 <- read.delim(wam_analysis_32hpi, header=TRUE)
df_36 <- read.delim(wam_analysis_36hpi, header=TRUE)

df_28C1 <- get_percent_modified(df_28, "28C1_read_depth")
df_28C2 <- get_percent_modified(df_28, "28C2_read_depth")
df_28K1 <- get_percent_modified(df_28, "28K1_read_depth")
df_28K2 <- get_percent_modified(df_28, "28K2_read_depth")

df_32C1 <- get_percent_modified(df_32, "32C1_read_depth")
df_32C2 <- get_percent_modified(df_32, "32C2_read_depth")
df_32K1 <- get_percent_modified(df_32, "32K1_read_depth")
df_32K2 <- get_percent_modified(df_32, "32K2_read_depth")

df_36C1 <- get_percent_modified(df_36, "36C1_read_depth")
df_36C2 <- get_percent_modified(df_36, "36C2_read_depth")
df_36K1 <- get_percent_modified(df_36, "36K1_read_depth")
df_36K2 <- get_percent_modified(df_36, "36K2_read_depth")

wams <- cbind(df_28C1, df_28C2, df_28K1, df_28K2,
              df_32C1, df_32C2, df_32K1, df_32K2,
              df_36C1, df_36C2, df_36K1, df_36K2)
...
time <- factor(rep(c("28", "32", "36"), each=4, times=1))
colnames(wams) <- row_names

# keep only rows that have a >=0 wam in every sample
# df_no_allzero <- wams[rowSums(wams == 0) != ncol(wams), ]
df_no_anyzero <- wams[!apply(wams == 0, 1, any), ]

pca <- prcomp(t(df_no_anyzero), scale. = TRUE, center = TRUE)
...
gois_clean <- sub("\\.[0-9]+$", "", rownames(loadings))
loadings$gene <- gois_clean
...
# WAM SPECIFIC (since our gene ids are transcript)
